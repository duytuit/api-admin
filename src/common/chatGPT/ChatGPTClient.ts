...
export default class ChatGPTClient {
  ...
  constructor(apiKey, options: any, cacheOptions: any) {
    this.apiKey = apiKey;
  ...
  async getCompletion(prompt, onProgress) {
    ...
    const url =
      this.options.reverseProxyUrl || 'https://api.openai.com/v1/completions';
    if (debug) {
      console.debug();
      console.debug(url);
  ...
  async sendMessage(message, opts: any) {
    try {
      const conversationId =
        opts.conversationId || Helper.generateRamdomByLength(6);
      const parentMessageId =
        opts.parentMessageId || Helper.generateRamdomByLength(6);
      ...
      const userMessage = {
        id: Helper.generateRamdomByLength(6),
        parentMessageId,
        role: 'User',
        message,
      };
  ...
  async buildPrompt(messages, parentMessageId) {
    ...
    let currentTokenCount = this.getTokenCount(
    ...
    this.modelOptions.max_tokens = Math.min(
      this.maxContextTokens - numTokens,
      this.maxResponseTokens,
    );
   ...
   */
  private getMessagesForConversation(messages, parentMessageId) {
